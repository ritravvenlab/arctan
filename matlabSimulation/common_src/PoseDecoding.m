...
function TagPose = TD_getRelativeTandR(TagSize,Fx,Fy,cx,cy,TD_struct)
...
yaw = StandardRad(gcordicatan2(wRo(2,1),wRo(1,1),luts));
c = cos(yaw);
s = sin(yaw);
pitch = StandardRad(gcordicatan2(-wRo(3,1), wRo(1,1)*c + wRo(2,1)*s,luts));
roll  = StandardRad(gcordicatan2(wRo(1,3)*s - wRo(2,3)*c, -wRo(1,2)*s + wRo(2,2)*c,luts));
end